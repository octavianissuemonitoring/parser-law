...
def run_scraper_and_import(url_str: str, link_id: int):
    ...
    try:
        ...
        # Get list of existing CSV files before scraping
        existing_csvs = set(glob.glob("/app/rezultate/*.csv"))
        
        logger.info(f"📥 Starting scraper for URL: {url_str}")
        logger.info(f"📝 Created links file: {links_file_path}")
        
        # Run scraper using v2 library (src)
        try:
            import sys
            ...
            scraper = LegislationScraper(
                ...
                output_dir="/app/rezultate"
            )
            scraper.run(delay_seconds=2.0)
        except Exception as e:
            error_msg = f"Scraper failed: {str(e)[:500]}"
        ...
        # Find new CSV files generated by scraper
        current_csvs = set(glob.glob("/app/rezultate/*.csv"))
        new_csvs = current_csvs - existing_csvs
        
        if not new_csvs:
            error_msg = "No CSV files generated by scraper"
            logger.error(f"❌ {error_msg}")
            update_link_status_sync(link_id, LinkStatus.FAILED, 0, error_msg)
            return
        
        logger.info(f"📦 Found {len(new_csvs)} new CSV files to import")
        
        # Move new CSV files to temporary directory for import
        import_dir = "/app/temp_import"
        ...
        # Move new files (CSV and MD) to temp directory
        for csv_file in new_csvs:
            md_file = csv_file.replace('.csv', '.md')
            shutil.move(csv_file, import_dir)

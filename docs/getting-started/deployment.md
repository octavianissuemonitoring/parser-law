...
### 1.6. Instalare Python și dependențe
```bash
...
### 2.4. Instalare dependențe Python pentru scraper
```bash
# Instalare dependențe din requirements-scheduler.txt
cd /opt/parser-law
pip3 install -r requirements-scheduler.txt --break-system-packages

# Verificare instalare
python3 -c "import pandas, bs4, requests; print('Dependencies OK')"
```

**Note:** Flag-ul `--break-system-packages` este necesar pe Ubuntu 24.04+ pentru a instala pachete Python la nivel de sistem.
...
**Note:** Fișierul `docker-compose.yml` este pre-configurat cu:
- DNS servers (8.8.8.8, 8.8.4.4) pentru acces internet din container
- Directory `/app/rezultate/` intern (fără bind mount pentru a evita probleme de permisiuni)
- Build context corect pentru Dockerfile
...
# IMPORTANT: Restart API pentru a reconecta la database cu schema actualizată
cd /opt/parser-law
docker compose -f db_service/docker-compose.yml restart api

# Așteaptă 5 secunde pentru restart complet
sleep 5
```

### 3.6. Pregătire pentru primul import

**IMPORTANT:** Nu rulați scraper-ul manual pe host înainte de a adăuga linkuri prin API. Background task-ul detectează CSV-uri noi prin comparare before/after, deci CSV-urile vechi pot cauza eroarea "No CSV files generated".

```bash
# Asigură-te că directorul rezultate din container este gol
cd /opt/parser-law
docker compose -f db_service/docker-compose.yml exec api ls -la /app/rezultate/

# Dacă există fișiere vechi, șterge-le
docker compose -f db_service/docker-compose.yml exec api rm -f /app/rezultate/*
```

**Primul link trebuie adăugat prin API folosind endpoint-ul `/api/v1/links/process`:**
```bash
# Exemplu: adaugă și procesează un link
curl -X POST "https://legislatie.issuemonitoring.ro/api/v1/links/process?url=https://legislatie.just.ro/Public/FormaPrintabila/YOUR_LINK_ID" \
  -H "Content-Type: application/json"
```

### 3.7. Start Scheduler (opțional)
...
### Issue: Link-uri marcate "failed" cu "No CSV files generated by scraper"
**Cauză:** Directorul `/app/rezultate/` conține CSV-uri vechi de la teste manuale sau rulări anterioare. Background task-ul compară CSV-urile before/after scraping și nu detectează fișiere noi.

**Simptome:**
- Link status: "failed"
- Error message: "No CSV files generated by scraper"
- Scraper-ul funcționează manual în container
- CSV-uri vechi în `/app/rezultate/`

**Soluție:**
```bash
# 1. Verifică CSV-urile existente
docker exec legislatie_api ls -lh /app/rezultate/

# 2. Șterge CSV-urile vechi DOAR dacă nu au fost importate în DB
docker exec legislatie_api sh -c "rm -f /app/rezultate/*.csv /app/rezultate/*.md"

# 3. Reprocessează link-ul eșuat prin API
curl -X POST "https://legislatie.issuemonitoring.ro/api/v1/links/process?url=URL_LINK_ESUAT"

# 4. Așteaptă 1-2 minute și verifică status
curl https://legislatie.issuemonitoring.ro/api/v1/links/
```

**Prevenție:** Nu rulați `scraper_legislatie.py` manual pe host sau în container înainte de a adăuga linkuri prin API.

### Issue: No CSV files generated by scraper / ModuleNotFoundError
**Cauză principală:** Eroare SQL la procesarea link-ului (de exemplu, coloane lipsă) care împiedică rularea scraper-ului.
**Cauză secundară:** Dependențele Python pentru scraper nu sunt instalate (pandas, beautifulsoup4, openpyxl, etc.).

**Simptome:**
- Eroare în API: "No CSV files generated by scraper"
- Erori SQL în logs: "column articole.metadate does not exist"
- Directorul `rezultate/` este gol (doar dacă scraper-ul nu a fost rulat niciodată)
- Eroare: `ModuleNotFoundError: No module named 'pandas'`

**Soluție 1 - Restart API după modificări database:**
```bash
# După orice modificare a schemei database (ALTER TABLE, CREATE TABLE)
cd /opt/parser-law
docker compose -f db_service/docker-compose.yml restart api

# Verificare că API-ul pornește fără erori
docker compose -f db_service/docker-compose.yml logs api --tail 50
```

**Soluție 2 - Verificare și reparare dependențe:**
```bash
# Verifică dacă pandas este instalat în container
docker exec legislatie_api python3 -c "import pandas; print(pandas.__version__)"

# Dacă lipsește, reinstalează imaginea cu dependențele corecte
cd /opt/parser-law
docker compose -f db_service/docker-compose.yml down
docker compose -f db_service/docker-compose.yml up -d --build
```

**Soluție 3 - Instalare dependențe pe host (pentru scraper standalone):**
```bash
# Instalare pip dacă lipsește
sudo apt install python3-pip python3-dev build-essential -y

# Instalare dependențe scraper
cd /opt/parser-law
pip3 install -r requirements-scheduler.txt --break-system-packages

# Verificare instalare
python3 -c "import pandas; print(f'pandas {pandas.__version__} OK')"
python3 -c "import bs4; print(f'beautifulsoup4 OK')"
